...
If GitHub network requests (clones, fetches, submodules) fail due to restrictions or authentication issues, retry using a personal OAuth token stored in an environment variable:

```
https://<OAUTH_TOKEN>:x-oauth-basic@github.com/<repo_path>.git
```
