...
See `[REDACTED]`, `[REDACTED]`, or `[REDACTED]` for examples. Place new
...
without relying on hallucinated data or online searches.
 ...
 Codex must never modify the engine's quantum systems based on reference data. Manifests like `[REDACTED]` are
 interpretive only and may not override the custom Collapse Oracle logic.

 Current manifest categories include: `physics/`, `visuals/`, `biology/`, `chemistry/`, `ai/`, `audio/`, `entropy/`, and `other/`.
